...
% R is initialized as a n*n diagonal matrix with all diagonals=2
...
x=zeros(m,T);
y=zeros(n,T);
% sqrtRdiag is a vector storing the sqrt of elements of R main diagonal
sqrtRdiag=sqrt(diag(R)); % diag(R) is to get the elements of main diagonal
for t=1:T
    if t==1
...
if option == 1
    save(strcat('./p-',int2str(n),'-d-',int2str(m),'-T-',int2str(T),'-data-regA-sparseC.mat'));
end
if option == 2
    save(strcat('./p-',int2str(n),'-d-',int2str(m),'-T-',int2str(T),'-data-sparseA-smoothC.mat'));
end
if option == 3
    save(strcat('./p-',int2str(n),'-d-',int2str(m),'-T-',int2str(T),'-data-regA-smoothsparseC.mat'));
end
